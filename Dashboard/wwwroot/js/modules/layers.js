define(["jquery-ui"], function($) {
	...
	Layers.initialize = function(namespace) {
		...
		$("#layer-clear").on("click", function (e) {
			var selectedLayer = Layers.get_active();
			if (selectedLayer) {
				var id = selectedLayer.id;
				if (confirm("Remove all tiles from this layer?")) {
	...
	Layers.add = function(e, name) {
		...
		$("#layerlist li").removeClass("active");
		$("#layerlist").append("<li class='active' data-id='" + id + "'> " + name + "</li>");
	...
	Layers.rename = function(e) {
		...
		if (!new_name || new_name.length < 3) {
			if (new_name) { alert("Name too short! Enter more than 3 characters!"); }
			return;
		}
		...
		$(Layers.contextTarget).html(" " + new_name + "<span class='icon-cog'></span>");
		$("#contextmenu").remove();
	};
	...
	Layers.open_contextmenu = function(e) {
		...
		$.get("/MapEditor/RenderPartial/_CmLayer", function (data) {
			$("body").append(data);
